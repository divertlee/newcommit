...
public:
	typedef __RBTreeIterator<T,Ref,Ptr> self;//const迭代器
	typedef __RBTreeIterator<T,T&,T*> iterator;//普通迭代器
	node* _node;

	__RBTreeIterator( node* node)//迭代器构造函数
		:_node(node)
	{}

	__RBTreeIterator(const iterator& s)//当传过来是const迭代器时，这里是拷贝构造；当传普通迭代器过来时，这里是普通迭代器迭代器构造函数--把普通迭代器构造const迭代器
		//传普通迭代器过来，构造一个const迭代器返回回去---解决map的set的key不能修改--set传普通迭代器过来，传const迭代器回去
		:_node(s._node)
	{}
