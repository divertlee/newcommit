...
1. 客户端向服务器发送数据，发了1000条报文，但服务器只收到了1条报文，意味着999条报文丢失了。而大部分的报文丢失意味着可能并不是网络问题，不能通过重传机制重新发送报文，这样会造成大量的报文在网络中堵塞，只会加重网络的故障，导致网络堵塞问题。因此应该不使用重传机制，环节网络的压力，网络有自己的恢复机制，等待网络的恢复。
2. 因此引入哟一个概念“拥塞窗口”，该窗口是发送端根据网络接收能力定义的发送量。发送开始时定义拥塞窗口大小为1字节，每收到一个ACK应答，拥塞窗口+1。每次发送数据包时，将拥塞窗口和接收端反馈的接收缓冲区剩余空间（接收能力-反馈窗口）做比较，取较小的作为发送端的滑动窗口大小。
3. 上面的做法被称为慢启动。
4. 慢启动算法：先是慢开始，拥塞窗口已指数型增长到设定好的阈值，过了阈值之后线性增长，然后去探测超时重发的网络拥塞的拥塞窗口大小（最大）。探测到后已最大的拥塞窗口%2作为新的阈值。同时拥塞窗口大小重新为1，然后进行新一轮的拥塞窗口探测。整个过程会呈现出周期性摆动，但也不一定，因为网络同时在波动，因此整个过程就是一种探测行为。
5. 该方法即保证了不失可靠性的同时提高了效率

### 延迟应答

客户端发送报文过来，服务器不立即发送响应。存在上层处理数据的速度很快的情况，因此可以设定一个时间，在时间内等待上层处理完数据，这样服务器响应客户端的接收缓冲区大小就会比较大，即加大了单次通信的吞吐量。因此也能做到收到客户端两次数据，响应一次应答。但延迟应答的时间不能超过超时重传的时间。
